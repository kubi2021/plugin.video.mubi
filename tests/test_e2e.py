...
class TestAddonEntryPoint:
    ...
    @patch('resources.lib.migrations.add_mubi_sources')
    @patch('resources.lib.migrations.mark_first_run')
    def test_addon_first_run_e2e(self, mock_mark_first_run, mock_add_sources,
    ...
    def test_addon_sync_films_e2e(self, mock_kodi_environment):
        ...
        request_info = self.simulate_addon_execution(
            handle=123,
            base_url="plugin://plugin.video.mubi/",
            params="?action=sync_films_locally"
        )

        assert request_info['action'] == 'sync_films_locally'
        ...
        with patch('xbmcvfs.translatePath') as mock_translate:
            ...
            # Test that sync_films_locally method exists and can be called
            assert hasattr(nav_handler, 'sync_films_locally')
...
class TestCompleteUserJourneys:
    ...
    def test_new_user_complete_journey(self, journey_setup):
        ...
        with patch('xbmcaddon.Addon') as mock_addon_class, \
            ...
            mubi.film_library.add_film(film)
            assert len(mubi.film_library) == 1
            ...
            with patch.object(film, 'create_nfo_file') as mock_nfo, \
                ...
                result = mubi.film_library.prepare_files_for_film(
                ...
                expected_folder = paths['temp_dir'] / "films" / film.get_sanitized_folder_name()
                assert expected_folder.exists()
                
            # Step 6: Mark First Run Complete
            from resources.lib.migrations import mark_first_run
            mark_first_run(mock_addon)
            mock_addon.setSetting.assert_called_with('first_run_completed', 'true')
