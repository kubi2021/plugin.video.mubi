...
- Every `datetime.fromisoformat` that feeds a datetime comparison must normalise naive→UTC (`if dt.tzinfo is None: dt = dt.replace(tzinfo=timezone.utc)`) — a tz-less string parses *successfully* as a naive datetime (no `ValueError` to catch), then raises `TypeError: can't compare offset-naive and offset-aware datetimes` at the comparison, outside the parse `try`. The digest's three parsers (`get_earliest_availability`, `get_first_available`, `get_latest_expiration`) all route through one canonical helper, `generate_weekly_digest._parse_iso_utc`; `scraper.py` `_earliest_started_availability` mirrors it inline — keep the two identical. The helper also type-guards (`isinstance(value, str)`): a non-string date (e.g. a number from schema/upstream drift) hits `.endswith` and raises `AttributeError`, which is *not* caught by a `ValueError`/`TypeError` clause and would abort the whole digest (hostile-audit F3, PR 77). PR 76 dropped the naive→UTC guard on the field the digest filters on; PR 77's initial parsers caught three different exception sets before this was unified.
- The weekly digest features a film only when its frozen `first_available_at` (the window's `available_at`, set by `scraper._earliest_started_availability`) falls inside the 7-day lookback — it keys on when the film *went live*, not when the scraper first *saw* it. So a genuinely-new film first observed as playable more than 7 days after its `available_at` is stamped with an already-aged date, frozen, and then silently missed **forever** (hostile-audit F1, PR 77). This is an accepted, narrow risk: the deep sync covers all countries weekly, so it only bites after a deep-sync failure (that week the digest runs on stale data anyway) or a film exclusive to a country skipped by every shallow sync for a full week. Closing it properly needs cross-run digest state (a persistent already-featured id set so the lookback can widen without duplicating) — do that as its own change, not a bolt-on; do NOT "fix" it by stamping observation time, which re-creates the first-backfill blow-up `first_available_at` was designed to avoid.
