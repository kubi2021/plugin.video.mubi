...
- Every `datetime.fromisoformat` that feeds a datetime comparison must normalise naive→UTC (`if dt.tzinfo is None: dt = dt.replace(tzinfo=timezone.utc)`) — a tz-less string parses *successfully* as a naive datetime (no `ValueError` to catch), then raises `TypeError: can't compare offset-naive and offset-aware datetimes` at the comparison, outside the parse `try`. The three parsers in `generate_weekly_digest.py` (`get_earliest_availability`, `get_first_seen`, `get_latest_expiration`) are the canonical set — keep them identical. PR 76 copied the Z-suffix handling into `get_first_seen` but dropped the guard, leaving a latent whole-digest crash on the field the digest now filters on.
