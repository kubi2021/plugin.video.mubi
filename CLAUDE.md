...
- Every `datetime.fromisoformat` that feeds a datetime comparison must normalise naive→UTC (`if dt.tzinfo is None: dt = dt.replace(tzinfo=timezone.utc)`) — a tz-less string parses *successfully* as a naive datetime (no `ValueError` to catch), then raises `TypeError: can't compare offset-naive and offset-aware datetimes` at the comparison, outside the parse `try`. The parsers in `generate_weekly_digest.py` (`get_earliest_availability`, `get_first_available`, `get_latest_expiration`) plus `scraper.py` `_earliest_started_availability` are the canonical set — keep the normalisation identical. PR 76 copied the Z-suffix handling into the digest's new parser but dropped the guard, leaving a latent whole-digest crash on the field the digest filters on.
