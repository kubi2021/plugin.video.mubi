"""
    ...
    - Folder names can use underscores (e.g., plugin_video_mubi) for Python compatibility
    - Zip files will use the addon ID from addon.xml (e.g., plugin.video.mubi-1.1.1.zip)
    - This ensures Kodi compatibility while maintaining clean Python package structure
"""
...
class Generator:
    ...
    def _create_zip(self, folder, addon_id, version):
        ...
        Uses the addon_id from addon.xml (with dots) for zip naming,
        regardless of folder name (which may have underscores).
        """
        ...
        if not os.path.exists(final_zip):
            ...
            for root, dirs, files in os.walk(addon_folder):
                ...
                # Replace folder name with addon_id in the archive path
                # This ensures the zip contains the correct addon ID structure
                if archive_root.startswith(os.sep + folder):
                    archive_root = archive_root.replace(os.sep + folder, os.sep + addon_id, 1)
